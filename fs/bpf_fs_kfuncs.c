					  ...
					  const struct bpf_dynptr *value_p)
{
	...
	/*
	 * Allocate a single buffer holding the value followed by a copy of
	 * the name. xattr->name and xattr->value both point into this
	 * buffer so they share its lifetime: the framework's existing
	 * cleanup path (kfree(xattr->value) in security_inode_init_security)
	 * frees both at once, after the filesystem's initxattrs callback
	 * has run. Copying the name avoids a use-after-free if the BPF
	 * program (and the map memory backing name__str) is released
	 * before initxattrs completes.
	 */
