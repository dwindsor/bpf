...
static void test_init_inode_xattr(void)
{
	...
	char value_out[64];
	/*
	 * Use tmpfs: the file must be created on a filesystem that calls
	 * security_inode_init_security() with an initxattrs callback, or the
	 * inode_init_security hook never fires (e.g. the 9p CI rootfs).
	 */
	...
	/*
	 * The hook only fires on filesystems that call
	 * security_inode_init_security() (e.g. not on a 9p rootfs).
	 */
