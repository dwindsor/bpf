...
static void test_set_file_xattr(void)
{
	...
	/* Arm only after the skeleton has attached: libbpf itself opens files while
	 * loading, and this test is about the explicit target below. */
	skel->bss->armed = true;
	fd = open(path, O_RDWR);
	...
	ASSERT_EQ(skel->bss->set_result, 0, "set_file_xattr_result");
	ASSERT_EQ(skel->bss->forbidden_result, -EPERM, "set_file_xattr_forbidden");
