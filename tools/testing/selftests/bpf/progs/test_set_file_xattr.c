...
static const char xattr_name[] = "security.bpf.file_test";
static const char forbidden_name[] = "security.selinux";
...
int BPF_PROG(test_set_file_xattr, struct file *file)
{
	...
	/* One shot. The kfunc itself can cause filesystem activity and the
	 * test process opens more files while reading results.
	 */
	armed = false;
	hook_ran = true;
